...
Token* getToken(void) {
  ...
  case CHAR_TIMES:
    ...
    cn = colNo;
    readChar();
    // Check if the next character is also CHAR_TIMES for exponential computation (**)
    if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_TIMES)) {
      readChar();
      return makeToken(SB_EXP, ln, cn); // SB_EXP represents the exponential computation (**)
    } else {
      return makeToken(SB_TIMES, ln, cn); // Single CHAR_TIMES represents multiplication (*)
    }
  case CHAR_SLASH:
