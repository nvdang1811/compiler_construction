...
void checkAssignment(Object** varList, int varCount, Type** exprList, int exprCount) {
  if (varCount != exprCount) {
    error(ERR_INVALID_ASSIGNMENT, currentToken->lineNo, currentToken->colNo);
  }

  for (int i = 0; i < varCount; i++) {
    checkTypeEquality(varList[i]->varAttrs->type, exprList[i]);
  }
}
