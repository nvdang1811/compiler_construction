...
void compileVariableList(Object* varList[], int* varCount) {
...
void compileExpressionList(Type* exprList[], int* exprCount) {
  exprList[(*exprCount)] = compileExpression();
  (*exprCount)++;
  while (lookAhead->tokenType == SB_COMMA) {
    eat(SB_COMMA);
    exprList[(*exprCount)] = compileExpression();
...
void compileAssignment(void) {
  ...
  compileVariableList(varList, &varCount);

  eat(SB_ASSIGN);

  compileExpressionList(exprList, &exprCount);

  // Check assignment
  checkAssignment(varList, varCount, exprList, exprCount);
}
